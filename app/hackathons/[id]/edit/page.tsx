...
import { createHackathon, updateHackathon, getHackathonById } from "@/lib/hackathons"
...
export default function EditHackathonPage() {
    ...
    const fetchIt = async () => {
      try {
        ...
        if (h) {
          ...
          setIsOnline(h.isOnline || false);
          setBannerPreview(h.bannerURL || "");
          setMinTeamSize(h.minTeamSize);
          setMaxTeamSize(h.maxTeamSize);
          setStartDate(h.startDate ? new Date((h.startDate as any).toDate ? (h.startDate as any).toDate() : h.startDate).toISOString().slice(0, 16) : "");
          setEndDate(h.endDate ? new Date((h.endDate as any).toDate ? (h.endDate as any).toDate() : h.endDate).toISOString().slice(0, 16) : "");
          setRegistrationDeadline(h.registrationDeadline ? new Date((h.registrationDeadline as any).toDate ? (h.registrationDeadline as any).toDate() : h.registrationDeadline).toISOString().slice(0, 16) : "");
          setSubmissionDeadline(h.submissionDeadline ? new Date((h.submissionDeadline as any).toDate ? (h.submissionDeadline as any).toDate() : h.submissionDeadline).toISOString().slice(0, 16) : "");
          setRules(h.rules?.length ? h.rules : [""]);
          setPrizes(h.prizes?.length ? h.prizes : [""]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await updateHackathon(hackathonId, hackathonData)
      toast.success("Hackathon updated successfully!")
      router.push(`/hackathons/${hackathonId}`)
    } catch (error: any) {
      console.error("Error updating hackathon:", error)
      toast.error(error.message || "Failed to update hackathon. Please try again.")
    } finally {
      setSubmitting(false)
    ...
    <div className="min-h-screen bg-background py-12">
      <div className="mx-auto max-w-4xl px-4 md:px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <h1 className="text-4xl font-bold text-primary mb-4">Edit Hackathon</h1>
          <p className="text-muted-foreground">Update details for the competition</p>
        </motion.div>
